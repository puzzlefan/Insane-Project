...
RotaryEncoder::RotaryEncoder(int P1, int P2){//constructor
  ...
  //pinMode(lBTwo,INPUT);
  *actStat[0] = digitize(analogRead(lBOne));
  *actStat[1] = digitize(analogRead(lBTwo));
...
bool RotaryEncoder::digitize(int analog){
    if(analog > 400){
        return true;
    }
    else{
...
void RotaryEncoder::Check(){
    lBOneNewStat=digitize(analogRead(lBOne));
    if(lBOneNewStat!=lBOneStat){
      lBOneStat = lBOneNewStat;
    }
    determineDir();
    lBTwoNewStat=digitize(analogRead(lBTwo));
...
RotaryEncoder first(0,1);
RotaryEncoder second(2,3);
RotaryEncoder third(4,5);
RotaryEncoder fourth(6,7);
...
void debugOutput(){
    Serial.println(tire[0]->getSteps());    
    //Serial.print(tire[0]->digitize(analogRead(0)));
    //Serial.print(",");
    //Serial.print(tire[0]->digitize(analogRead(1)));
    //Serial.print(",");
    //Serial.print(tire[0]->digitize(analogRead(1)));
    //Serial.print(",");
